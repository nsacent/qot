...
class ListingLifecycleTests(APITestCase):
    ...
    def test_active_listing_cannot_be_renewed_before_expiry(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertFalse(response.data["renewal_available"])
        self.assertGreater(response.data["seconds_remaining"], 0)
        self.assertEqual(active.status, Listing.STATUS_ACTIVE)
        self.assertEqual(active.expires_at, expires_at)
